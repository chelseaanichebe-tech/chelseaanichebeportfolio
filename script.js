...
document.addEventListener('click', (e) => {
  if (e.target.matches('[data-close]') || e.target === pm || e.target === rm) {
...
function applySettings(s) {
  ...
  if (meta && s.siteDescription) meta.setAttribute('content', s.siteDescription);
  ...
  if (Array.isArray(s.aboutParagraphs) && s.aboutParagraphs.length) {
    ...
    if (box) {
      box.innerHTML = s.aboutParagraphs
        .map((p) => `<p>${escapeHtml(typeof p === 'string' ? p : p.paragraph || p)}</p>`)
        .join('');
    }
  }
...
function renderServices(data) {
  ...
  el.innerHTML = items
    .map(
      (i) =>
        `<article><span>${escapeHtml(i.number)}</span><div><h3>${escapeHtml(
          i.title
        )}</h3><p>${escapeHtml(i.description)}</p></div></article>`
    )
    .join('');
}
      ...
      if (gallerySrcs.length) {
        ...
        galleryHtml = `
          <div class="project-carousel" data-total="${gallerySrcs.length}">
            ...
            ${
              showNav
                ? `<p class="carousel-counter"><span class="carousel-current">1</span>/${gallerySrcs.length}</p>`
                : ''
            }
          </div>`;
      } else {
        galleryHtml = `<div class="modal-gallery placeholder-gallery">${escapeHtml(
  ...
  el.innerHTML = items
    .map((t, idx) => {
      ...
      return `<article class="testimonial-card" data-index="${idx}" tabindex="0" role="button" aria-expanded="false">
        ...
        <p class="quote-preview">${preview}</p>
        <p class="quote-full">${full}</p>
        <div class="t-meta">
          ${photo}
          <div>
        ...
        <button type="button" class="t-toggle" aria-hidden="true">Read more</button>
      </article>`;
    })
    .join('');
